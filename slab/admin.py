...
class UserSlabAdmin(admin.ModelAdmin):
    list_display = ('get_usercode', 'slab')
    search_fields = ('user__usercode', 'slab__slab_name')

    def get_usercode(self, obj):
        return obj.user.usercode
    get_usercode.short_description = 'User Code'

@admin.register(EarningHistory)
class EarningHistoryAdmin(admin.ModelAdmin):
    list_display = ('get_usercode', 'earning_type', 'earning_amount', 'currency', 'earning_from', 'datetime')
    search_fields = ('user__usercode', 'earning_type', 'earning_from')
